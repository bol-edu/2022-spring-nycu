...
reg [DATA_WIDTH - 1 : 0] mem_accum [accum_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_accum [ (accum_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * accum_DEPTH -1 : 0] = '{default : 'hz};
reg accum_read_data_finish;
reg [DATA_WIDTH - 1 : 0] mem_accum_clr [accum_clr_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_accum_clr [ (accum_clr_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * accum_clr_DEPTH -1 : 0] = '{default : 'hz};
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_accum_clr_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_accum_clr[write_accum_clr_count * four_byte_num  + i]!==accum_clr_data_tmp_reg) begin
                    write (accum_clr_data_in_addr + write_accum_clr_count * four_byte_num * 4 + i * 4, accum_clr_data_tmp_reg, write_accum_clr_resp);
                    image_mem_accum_clr[write_accum_clr_count * four_byte_num + i]=accum_clr_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
initial begin : write_accum_file_proc 
  ...
  integer factor; 
  integer transaction_idx; 
  reg [accum_c_bitwidth - 1 : 0] tmp_cache_mem; 
  reg [ 100*8 : 1] str;
  reg [63:0] bin_data;
  integer i; 
  transaction_idx = 0; 
  ...
  while(1) begin 
      ...
      for (i = 0; i < (accum_DEPTH - accum_DEPTH % factor); i = i + 1) begin
          if (factor == 4) begin
              if (i%factor == 0) begin
                  tmp_cache_mem = mem_accum[i/factor][7:0];
              end
              if (i%factor == 1) begin
                  tmp_cache_mem = mem_accum[i/factor][15:8];
              end
              if (i%factor == 2) begin
                  tmp_cache_mem = mem_accum[i/factor][23:16];
              end
              if (i%factor == 3) begin
                  tmp_cache_mem = mem_accum[i/factor][31:24];
              end
              $fdisplay(fp,"0x%x",tmp_cache_mem);
          end
          if (factor == 2) begin
              if (i%factor == 0) begin
                  tmp_cache_mem = mem_accum[i/factor][15:0];
              end
              if (i%factor == 1) begin
                  tmp_cache_mem = mem_accum[i/factor][31:16];
              end
              $fdisplay(fp,"0x%x",tmp_cache_mem);
          end
          if (factor == 1) begin
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < accum_clr_DEPTH; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_accum_clr [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
