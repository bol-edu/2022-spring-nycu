...
#ifndef _hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W_H_
#define _hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W_memcore.h"

SC_MODULE(hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W) {
...
protected:
    ...
    // buffer instances
    hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W_memcore* buffer[BufferCount];
...
public:
    ~hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W();
    SC_CTOR(hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W_memcore_U_";
            name += szTmp;

            buffer[i] = new hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "hls_xfft2real_twid_rom_M_imag_V_RAM_AUTO_2R1W.trace");
            // ports
