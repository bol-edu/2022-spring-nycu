...
int main(int argc, const char* argv[]) {
    ...
    for(int r=0; r < dataAN; r++) {
        for(int c = r+1; c < dataAN; c++) {
            errA += (LU_golden[r][c] - LU_mm[r][c]) * (LU_golden[r][c] - LU_mm[r][c]);
        }
    }
    errA = std::sqrt(errA) / (dataAN * dataAN);
