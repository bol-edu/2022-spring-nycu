...
//			sizeof(int) * M * N, NULL, NULL);
	...
	output_max_index = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, NULL);
...
//	}
	...
	err = clEnqueueWriteBuffer(commands, output_max_index, CL_TRUE, 0,
				sizeof(int), max_index, 0, NULL, NULL);
		if (err != CL_SUCCESS) {
			printf("Error: Failed to write to source array a!\n");
			printf("Test failed\n");
			return EXIT_FAILURE;
	...
	err = 0;
	printf("set arg 1 \n");
	err = clSetKernelArg(kernel, 0, sizeof(int), &host_N);
	if (err != CL_SUCCESS) {
	...
	printf("set arg 2 \n");
	err |= clSetKernelArg(kernel, 1, sizeof(int), &host_M);
	if (err != CL_SUCCESS) {
	...
	printf("set arg 3 \n");
	err |= clSetKernelArg(kernel, 2, sizeof(cl_mem), &input_query);
	if (err != CL_SUCCESS) {
	...
	printf("set arg 4 \n");
	err |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &input_database);
	if (err != CL_SUCCESS) {
	...
	printf("set arg 5 \n");
	err |= clSetKernelArg(kernel, 4, sizeof(cl_mem), &output_direction_matrixhw);
	if (err != CL_SUCCESS) {
	...
	printf("set arg 6 \n");
	err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &output_max_index);
	if (err != CL_SUCCESS) {
	...
	cl_event readDirections, readMax;
